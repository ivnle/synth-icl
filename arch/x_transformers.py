...
class XTransformersModel(nn.Module):
    def __init__(self, cfg, embedder, head):
        super().__init__()
        if "x_encoder_only" in cfg.model and cfg.model.x_encoder_only:
            self.model = TransformerWrapper(
                ...
                attn_layers=Encoder(
                    ...
                    rotary_xpos=cfg.model.rotary_xpos,  # modified rotary to extrapolate well beyond length at which it was trained
                    use_rmsnorm=cfg.model.use_rmsnorm,
                    ff_swish=cfg.model.ff_swish,
                ),
            )
            self.model_type = "encoder-only"
        elif "x_decoder_only" in cfg.model and cfg.model.x_decoder_only:
            self.model = TransformerWrapper(
                num_tokens=1,
                max_seq_len=cfg.model.max_seq_len,
                attn_layers=Decoder(
                    ...
                    depth=cfg.model.n_layer,
                    heads=int(cfg.model.n_heads),
                    attn_kv_heads=int(cfg.model.n_kv_heads),
                    rotary_xpos=cfg.model.rotary_xpos,  # modified rotary to extrapolate well beyond length at which it was trained
                    use_rmsnorm=cfg.model.use_rmsnorm,
                    ff_swish=cfg.model.ff_swish,
                    attn_flash=cfg.model._name_ == "llama-x",
                ),
            )
            self.model_type = "decoder-only"
        elif "x_encoder_decoder" in cfg.model and cfg.model.x_encoder_decoder:
            self.encoder_model = TransformerWrapper(
                num_tokens=1,
                max_seq_len=cfg.model.max_seq_len,
                attn_layers=Encoder(
                    dim=cfg.model.d_model,
                    depth=int(cfg.model.n_layer / 2),
                    heads=int(cfg.model.n_heads),
                    rel_pos_bias=cfg.model._name_ == "t5-x",
                ),
            )
            self.decoder_model = TransformerWrapper(
                num_tokens=1,
                max_seq_len=cfg.model.max_seq_len,
                attn_layers=Decoder(
                    dim=cfg.model.d_model,
        ...
        elif cfg.model._name_ == "x-encoder":
            self.model_type = cfg.model._name_
            self.model = TransformerWrapper(
                num_tokens=1,
                **cfg.model.twrap_kwargs,
                attn_layers=Encoder(**cfg.model.attn_kwargs),
            )
        elif cfg.model._name_ == "x-decoder":
            self.model_type = cfg.model._name_
            self.model = TransformerWrapper(
                num_tokens=1,
                **cfg.model.twrap_kwargs,
                attn_layers=Decoder(**cfg.model.attn_kwargs),
            )
        elif cfg.model._name_ == "x-encoder-decoder":
            self.model_type = cfg.model._name_
            self.encoder_model = TransformerWrapper(
                num_tokens=1,
                **cfg.model.twrap_kwargs,
            ...
            self.decoder_model = TransformerWrapper(
                ...
                **cfg.model.twrap_kwargs,
                attn_layers=Decoder(
                    **cfg.model.attn_kwargs, **cfg.model.decoder_specific_kwargs
                ),
            )
        else:
    ...
    def forward(self, inputs_embeds):
        if self.model_type == "x-encoder-decoder":
            enc = self.encoder_model(inputs_embeds, return_embeddings=True)
            if self.cfg.model.pass_embed == True:
                output = self.decoder_model(
                    inputs_embeds, context=enc, return_embeddings=True
                )
            else:
                ...
                output = self.decoder_model(
                    dummy_input, context=enc, return_embeddings=True
                )
        else:
            output = self.model(inputs_embeds, return_embeddings=True)
        return output
...
class TransformerWrapper(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if len(x.shape) == 3:
            b, n, h, device, num_mem, emb_frac_gradient = (
                *x.shape,
                x.device,
        ...
        x, intermediates = self.attn_layers(
            x, mask=mask, mems=mems, cache=cache, return_hiddens=True, **kwargs
        )
