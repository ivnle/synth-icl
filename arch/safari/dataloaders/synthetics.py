...
from arch.safari.dataloaders.base import SequenceDataset
...
def generate_assoc_recall(
    ...
    # prevent using the same vocab for keys and values across examples
    rng.shuffle(vocab.non_special_vocab)
    ...
    # randomly pair keys with values
    rng.shuffle(values)
    kv_map = {tuple(k): v for k, v in zip(keys_multi, values)}
    # kv_map = {
...
class ICLDataModule(SequenceDataset):
    ...
    def setup(self, stage=None):
        train_tensor = eval_tensor = test_tensor = None
        ...
        if train_tensor is None or eval_tensor is None or test_tensor is None:
            ...
            for i, (example_count, valid_vocab) in enumerate(
                zip(
                    [self.num_examples, self.num_test_examples, self.num_test_examples],
                    [train_vocab, test_vocab, test_vocab],
                )
            ):
        ...
        self.dataset = {
            ...
            "eval": TensorDataset(eval_tensor[:, 0, :], eval_tensor[:, 1, :]),
        }
