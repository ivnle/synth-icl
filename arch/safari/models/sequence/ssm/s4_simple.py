...
def normalize_param(a, method, norm_const=None):
    if method == "l1":
        if norm_const is not None:
            return a / (
                (1 + norm_const) * torch.linalg.norm(a, ord=1, dim=2).unsqueeze(2)
            )
    ...
    def quadrature_method(self, u, horizon):
        ...
        if self.repr == "disc":
            ...
            base_term = (
                2
                * dt
                * torch.pow(a_.unsqueeze(2), zk)
                * torch.cos(self.theta.unsqueeze(2) * zk)
            )
        else:
...
class SimpleS4Wrapper(nn.Module):
    def __init__(
        ...
        **kernel_args,
        ...
        self.s4 = SimpleS4(
            nHippos=d_model, d_state=d_state, channels=channels, **kernel_args
        )
        # the mapping
