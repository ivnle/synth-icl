...
try:
    from arch.safari.ops.fftconv import fftconv_func
except ImportError:
    fftconv_func = None
...
class H3Conv(nn.Module):
    def __init__(
        ...
        **kernel_args,
        ...
        self.k_kernel = LongConvKernel(self.d_model, L=self.L, **kernel_args)
        self.k_D = nn.Parameter(torch.randn(self.d_model))
        self.kernel = LongConvKernel(self.d_model, L=self.L, **kernel_args)
