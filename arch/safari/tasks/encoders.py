...
import arch.safari.models.nn.utils as U
import arch.safari.utils as utils
import arch.safari.utils.config
from arch.safari.models.sequence.block import SequenceResidualBlock
from arch.safari.models.nn.components import Normalization
...
def _instantiate(encoder, dataset=None, model=None):
    ...
    dataset_args = arch.src.utils.config.extract_attrs_from_obj(
        dataset, *dataset_attrs.get(name, [])
    )
    model_args = arch.src.utils.config.extract_attrs_from_obj(
        model, *model_attrs.get(name, [])
    )
