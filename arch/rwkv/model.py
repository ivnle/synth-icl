...
if os.environ["RWKV_FLOAT_MODE"] == "bf16":
    wkv_cuda = load(
        ...
        sources=["arch/rwkv/cuda/wkv_op_bf16.cpp", "arch/rwkv/cuda/wkv_cuda_bf16.cu"],
        verbose=True,
        extra_cuda_cflags=[
            "-t 4",
            "-std=c++17",
    ...
    wkv_cuda = load(
        name=f"wkv_{T_MAX}",
        sources=["arch/rwkv/cuda/wkv_op.cpp", "arch/rwkv/cuda/wkv_cuda.cu"],
        verbose=True,
        extra_cuda_cflags=[
            "-res-usage",
            "--maxrregcount 60",
...
class RWKV_TimeMix_RWKV5_Preview(MyModule):
    def __init__(self, args, layer_id):
        ...
        with torch.no_grad():
            ...
            for h in range(self.n_head):
                decay_speed[h] = -8 + 7 * (h / (self.n_head - 1)) ** (
    ...
    if "r3" in os.environ["RWKV_MY_TESTING"]:
        ...
        def jit_func(self, x):
            ...
            r = (
                self.receptance(xr)
                .view(B, TT, self.n_head, self.head_size)
                .transpose(1, 2)
            )  # BTC -> BHTS
            k = (
                self.key(xk)
            ...
            v = (
                self.value(xv).view(B, TT, self.n_head, -1).transpose(1, 2)
            )  # BTC -> BHTS
        ...
        def jit_func_2(self, r, k, v, g, w, wk, wb, ws):
            ...
            x = (
                x.transpose(1, 2).contiguous().view(B * TT, H * S)
            )  # BHTS -> BTHS -> BTC
        ...
        def jit_func(self, x):
            ...
            v = (
                self.value(xv).view(B, TT, self.n_head, self.head_size).transpose(1, 2)
            )  # BTC -> BHTS
