...
class LightConvConfig(object):
    def __init__(self, **kwargs):
        self.args = argparse.ArgumentParser(prog="LightConvConfig")
        self.add_args(self.args)
        for k, v in kwargs.items():
            setattr(self.args, k, v)

        n_layer = self.args.decoder_layers
        kernel_size = [3, 7, 15]
        assert (n_layer > 0) and isinstance(n_layer, int)
        if n_layer in [1, 2, 3]:
            kernel_size = kernel_size[:n_layer]
        else:
            kernel_size = kernel_size + [31] * (n_layer - 3)
        setattr(self.args, "decoder_kernel_size_list", kernel_size)
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            ...
            help="if we just want decoder only",
        )


class LightConvModel(nn.Module):
    def __init__(self, args, embedder, head):
        super().__init__()
        self.args = args.args
        self.embedder = embedder
        self.head = head
        if self.args.decoder_only:
            self.model = LightConvLanguageModel.build_model(args.args, None)
        else:
            self.model = LightConv.build_model(args.args, None)

    def forward(self, x):
        if self.args.decoder_only:
            return self.model(src_embeds=x)[0]
        else:
            dummy_input = torch.zeros([x.size(0), 1], dtype=torch.long, device=x.device)
            return self.model(
                src_embeds=x,
                prev_output_embeds=dummy_input,
            )[0]


# @register_model("lightconv")
class LightConv(FairseqEncoderDecoderModel):
    # """
    ...
    def build_model(cls, args, task):
        ...
        print("Current encoder/decoder type", args.encoder_conv_type)
        ...
        encoder = LightConvEncoder(args, None, None)
        decoder = LightConvDecoder(args, None, None)
        return LightConv(encoder, decoder)

    def forward(
        self,
        src_embeds: Tensor,
        src_lengths: Optional[Tensor] = None,
        prev_output_embeds: Optional[Tensor] = None,
    ):
        """
        ...
        """
        encoder_out = self.encoder(src_embeds, src_lengths)
        decoder_out = self.decoder(prev_output_embeds, encoder_out=encoder_out)
        return decoder_out
...
class LightConvLanguageModel(FairseqLanguageModel):
    ...
    def build_model(cls, args, task):
        ...
        # make sure all arguments are present in older models
        base_architecture(args)
        print("Current decoder type", args.decoder_conv_type)
        ...
        decoder = LightConvDecoder(args, None, None, no_encoder_attn=True)
        return LightConvLanguageModel(decoder)

    def forward(self, src_embeds: Tensor, **kwargs):
        """
        (The forward method inherited from the base class has a **kwargs
        argument in its input, which is not supported in torchscript. This
        method overwrites the forward method definition without **kwargs.)
        ...
        Returns:
            tuple:
                ...
                - a dictionary with any model-specific outputs
        """
        decoder_out = self.decoder(prev_output_embeds=src_embeds, **kwargs)
        return decoder_out
...
class LightConvEncoder(FairseqEncoder):
    ...
    def forward(
        self, src_embeds: Tensor, src_lengths: Optional[Tensor] = None
        ...
        x = self.embed_scale * src_embeds  # TODO: do we need this self.embed_scale?
...
class LightConvDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        self.decoder_embedder = nn.Embedding(1, embed_dim, padding_idx=0)
        nn.init.normal_(self.decoder_embedder.weight, mean=0, std=embed_dim**-0.5)
    ...
    def forward(
        ...
        if encoder_out is not None:
            x = self.embed_scale * self.decoder_embedder(
                prev_output_embeds.contiguous()
            )  # TODO: do we need this self.embed_scale
        else:
            x = (
                self.embed_scale * prev_output_embeds
            )  # TODO: do we need this self.embed_scale
