...
class Curriculum:
    def __init__(self, args, split):
        ...
        if split in ["eval", "test"]:
            args.dims.start = args.dims.end
        else:
            assert split == "train"

        self.n_dims_truncated = args.dims.start
        self.n_points = args.points_train.start if split == "train" else args.points_val.start
        self.n_dims_schedule = args.dims
        self.n_points_schedule = args.points_train if split == "train" else args.points_val
        self.step_count = 0
